...
Each figure is the exact artifact embedded in `berlin_deck_v4` (slide noted), regenerated by the
locked pipeline (`analysis/run_all.py`). All numbers come from the committed CSVs; handles are
suppressed; every figure carries the *"Preliminary analysis — MICrONS proofreading annotators"*
stamp. **Cohorts:** *expert* = 8 part-time expert proofreaders (Janelia-trained); *proto-expert* =
