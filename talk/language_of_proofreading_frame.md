...
## 0. Live results — the anchor exists (pulled 2026-06 from `queue.neuvue.io`)

Real numbers, not a mock-up:

- **Scale:** a 40k-task sample spans **~78 proofreaders across 19 namespaces**
  (`automatedSplitGroup`, `multiSomaSplit`, `fullyProofread`, `somaSomaSplit`, …),
  with differstack history going back to 2021. This is the "lots of data, lots of
  people" slide, and it's true.
- **The differstack is a literal diff-history.** Each event is a
  diff-match-patch over the Neuroglancer state (`{patch, timestamp, change}`).
  The metaphor isn't a metaphor — proofreading *is* recorded as a sequence of
  state edits.
- **Utterance length & rhythm (robust):** in `fullyProofread`, 112 sessions /
  7,203 events / 13 proofreaders; **median 32 events per session** (up to 605);
  inter-event gap **median 2.1 s**; session span **median ~2 min** (p90 ~18 min).
- **First-pass vocabulary:** ~**70% navigation**, ~**10% annotation**, **<1%
  explicit segment edits** (expected — `fullyProofread` is validation, not
  splitting). Honest caveat below.
- **Dialects are real and visible:** action mix varies sharply by person —
  navigation share **28–83%**, annotation **4–33%**, label-entropy **0.8–1.6
  bits**. `akinwumi` is navigation-heavy (83%/4%); `katie` annotates far more
  (28%/33%). *That difference is the measurement the talk is about.*

Figures (in `notebooks/outputs/`, regenerated by the notebook; kept out of git
since they carry proofreader handles): vocabulary bar, inter-event timing,
grammar transition heatmap, dialect-distance (Jensen-Shannon) heatmap.

**The full behavior dictionary** — every captured behavior, the NG-state fields
behind it, task-type signatures, and proofreader archetypes — is in
[`proofreading_behavior_dictionary.md`](proofreading_behavior_dictionary.md).
...
- **~78 proofreaders · 7,203 edit events in one namespace · 19 namespaces** on
  NeuVue, from differstack history (Section 3 survey + Section 14).
- **Vocabulary:** ~70% navigation, ~10% annotation, <1% explicit segment edits
  in a validation namespace (Section 14a) — proofreading is mostly *looking*.
- **Grammar is real** — action→next-action transition structure, not uniform
  (Section 14b).
- **Dialects are measurable** — navigation share 28–83% across people; a
  Jensen-Shannon "dialect-distance" map separates them (Section 14c).
...
- **The vocabulary is first-pass.** `patch` is a *byte-level* diff-match-patch, so
  the changed field name is often outside the diff window. We label numeric-only
  diffs as navigation and key off `description`/`segments` tokens — good enough to
  show structure and dialects, but sharp **merge-vs-split** detection needs
  reconstructing before/after state and diffing the `segments` sets. That's the
  obvious next step, not done here.
- The differstack reflects what a given Neuroglancer build emits; some workflows
  (e.g. spelunker mode) send little or nothing, and **coverage is partial** —
  in `fullyProofread`, ~112 of ~1,200 sampled tasks carried a differstack.
